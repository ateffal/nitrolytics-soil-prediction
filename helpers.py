...
def prediction_nnet_dense_layers(data, features, target, lags=6, nodes_layers=[32, 32, 32], scale_data=True):
    ...
    df_features_unscaled = make_lags(
        data_[features], lags).dropna()

    df_target_unscaled = data_.loc[:, target].iloc[lags:]

    # scale data

    if scale_data:

        df_features = (df_features_unscaled - df_features_unscaled.min()) / \
            (df_features_unscaled.max()-df_features_unscaled.min())

        df_target = (df_target_unscaled - df_target_unscaled.min()) / \
            (df_target_unscaled.max()-df_target_unscaled.min())
    ...
    model = tf.keras.models.Sequential([tf.keras.layers.Dense(
        ...
        nodes_layers[0], input_shape=(None, 1, n_features * lags))])
    ...
    if len(nodes_layers) > 1:
        ...
        for l in nodes_layers[1:]:
            ...
            model.add(tf.keras.layers.Dense(l))  # was Dense(32)
    ...
    model.compile(
        ...
        loss="mae", optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3))
    ...
    model.fit(x=data_cmname_mktname_features,

              y=data_cmname_mktname_labels, epochs=100, verbose=0)
    ...
    plot_series(time, (df_target_unscaled.values, forecasts),

                (col_name_observed, col_name_predicted))

    forecasts_val = pd.DataFrame({col_name_observed: list(

        df_target_unscaled.values), col_name_predicted: list(forecasts[:, 0])})

    forecasts_val['diff'] = abs(

        forecasts_val[col_name_observed]-forecasts_val[col_name_predicted])

    forecasts_val['diff (%)'] = forecasts_val['diff'] / \
        forecasts_val[col_name_observed]
...
#     print('used data : ')
    ...
    rmse = mean_squared_error(
        ...
        forecasts_val[col_name_observed], forecasts_val[col_name_predicted], squared=False)
...
def predict_rnn(data_cmname_mktname_, features, target, lags=6, nodes_layers=[32, 32, 32], scale_data=True):
    ...
    rnn_model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.SimpleRNN(
            ...
            128, return_sequences=False, input_shape=input_shape_),
    ...
    history = rnn_model.fit(
        ...
        w2.train, validation_data=w2.val, epochs=epochs, verbose=0)
