...
def compare_models(data_rnn_, vars_to_use_, var_to_predict_, models, lags = 6, learning_rate = 1e-6, epochs = 500):
    ...
    if "NNET" in models:
        ...
        trained_models['NNET'] = nnet_model
    ...
    def make_pred(wdata):
      w_data = list(wdata.as_numpy_iterator()) # w2_test is a list of tuples : (features, target)
      temp = [b[1] for b in w_data] # take the target which is in the second position in each tuple
      temp_2 = []
      for t in temp:
        for e in list(t.flatten()):
          temp_2.append(e)

      pred_df = pd.DataFrame({'obs' : temp_2})

      RMSE = {}

      if "NNET" in models:
        pred_nnet = nnet_model.predict(wdata)[:,0]
        pred_df['pred nnet'] = pred_nnet
        
      if "RNN" in models:
        pred_rnn = rnn_model.predict(wdata)[:,0]
        pred_df['pred rnn'] = pred_rnn
        
      if "LSTM" in models:
        pred_lstm = lstm_model.predict(wdata)[:,0]
        pred_df['pred lstm'] = pred_lstm
        
      # unscale data
      pred_df = pred_df * (data_rnn_[var_to_predict_].max()-data_rnn_[var_to_predict_].min())
      pred_df = pred_df + data_rnn_[var_to_predict_].min()

      # calculating RMSEs on unscaled data
      if "NNET" in models:
    ...
    # return predictions for train data, RMSEs for train, validation and test data and the trained models
    return make_pred(w2.train), make_pred(w2.val), make_pred(w2.test), trained_models
...
def compare_models_old(data_rnn_, vars_to_use_, var_to_predict_, models, lags = 6, learning_rate = 1e-6, epochs = 500):
    data_rnn = data_rnn_.copy()
    data_rnn = data_rnn[vars_to_use_]
    data_rnn = (data_rnn - data_rnn.min())/(data_rnn.max()-data_rnn.min())
    n = len(data_rnn)
